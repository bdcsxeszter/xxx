...
class Game:
    def __init__(self):
        ...
        self.bg_st = PhotoImage(file="background_with_stars.gif")
        self.bg_win = PhotoImage(file="background_with_window.gif")
        self.bg_sh = PhotoImage(file="background_with_shelf.gif")
        w = self.bg.width()
        h = self.bg.height()
        count = 0
        draw_background = 0
        ...
        for x in range(0, 5):
            for y in range(0,5):
                if draw_background == 1:
                    ...
                    count = count + 1
                    if  count == 2 or count == 4 or count == 10:
        ...
        self.sprites = []
        self.running = True
        self.you_win = self.canvas.create_text(240,300,text='YOU WIN!',
                                               font=('Ariel',30),
                                               fill='#3aef05',
                                               state='hidden')
    ...
    def mainloop(self):
        while 1:
            if self.running == True:
                ...
                self.canvas.itemconfig(self.you_win, state='normal',
                                       fill='#3aef05')
                time.sleep(1)
                self.tk.update_idletasks()
                self.tk.update()
                self.canvas.itemconfig(self.you_win, fill='#f11b51')
                time.sleep(0.3)
            self.tk.update_idletasks()
            self.tk.update()
            time.sleep(0.01)
