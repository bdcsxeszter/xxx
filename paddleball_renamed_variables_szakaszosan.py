...
game_over=canvascreated.create_text(240,300,text='GAME OVER', font=('Ariel',30),fill='orange',state='hidden')
...
while 1:
    if ball.hit_bottom==False and ball.start==True:
        ...
        canvascreated.itemconfig(game_over, state='normal')
        time.sleep(1)
        tk.update_idletasks()
        tk.update()
        canvascreated.itemconfig(game_over, state='hidden')
        time.sleep(0.3)
    tk.update_idletasks()
    tk.update()
    time.sleep(0.01)
